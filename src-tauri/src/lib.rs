...
/// Starts an HTTP server for Supabase OAuth callbacks. Tries ports 54321-54330.
#[tauri::command]
fn start_fixed_oauth_server(window: tauri::Window) -> Result<u16, String> {
    ...
    std::thread::spawn(move || {
        let result = (|| -> Result<u16, String> {
            let listener = (54321u16..54331u16)
                .flat_map(|p| TcpListener::bind(("127.0.0.1", p)).ok())
                .next()
                .ok_or_else(|| "No available port in range 54321-54330".to_string())?;
